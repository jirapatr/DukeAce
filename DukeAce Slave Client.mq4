//+------------------------------------------------------------------+
//|                                         DukeAce Slave Client.mq4 |
//|                                        Copyright 2020, DukeAce., |
//|                                          https://www.dukeace.com |
//+------------------------------------------------------------------+
#property copyright "Copyright 2020, DukeAce.,"
...
#include <Zmq/Zmq.mqh>
#include <Grabweb.mqh>
string url="https://jovial-syntax-250218.firebaseio.com/User/"+(string)AccountNumber()+"/Status/.json";
//--- Inputs
...
const string app_name    = "DukeAce Slave Client";
...
void OnStart()
  {
   ...
   GrabWeb(url,reply);
//Print(reply);//string
   string index=(string)reply;
   int no=StringFind(index,"0",0);
   int yes=StringFind(index,"1",0);
   if(no==1)
     {
      MessageBox("Your account not activate","DukeAce");
      ExpertRemove();
     }
   if(yes==1)
     {
      MessageBox("Thank you for your choose our service","DukeAce");
     }
   if(yes!=1&&no!=1)
     {
      MessageBox("Please contact to register your account","DukeAce");
      ExpertRemove();
     }
   EACheck();
   StartZmqClient();
